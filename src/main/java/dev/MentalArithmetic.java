...
public class MentalArithmetic {
	...
	public static void main(String[] args) {
		...
		if (args.length == 2) {
			try {// The arguments should be integers.
		...
		} else {// no arguments so use defaults.
			limit = DEFAULTLIMIT;
		...
		try {
			...
			try {// Try to parse the number as an integer
	...
	public static Pair<String, Integer> generateQuestion(int limit, int length)
			throws PrologException, NoAnswerException {
		setup();
		...
		Term[] args = { new IntegerTerm(limit), new IntegerTerm(length),
				listTerm, answerTerm };
		// Construct the question
		CompoundTerm goalTerm = new CompoundTerm(AtomTerm.get("arithmetic"),
				args);
		...
		synchronized (interpreter)// so that this class is thread safe.
		{
			...
			if (rc == PrologCode.SUCCESS || rc == PrologCode.SUCCESS_LAST) {
				// Create the answer
				Pair<String, Integer> answer = new Pair<String, Integer>(null,
						0);

				// Get hold of the actual Terms which the variable terms point
				// to
					...
					if (list instanceof CompoundTerm) {
						...
						{// Turn it into a string to use.
							...
							throw new NoAnswerException(
									"List is not a list but is a CompoundTerm: "
											+ list);
						}
					} else {
						throw new NoAnswerException("List is not a list: "
								+ list);
					}
				} else {
					throw new NoAnswerException(
							"List null when it should not be null");
				}
				if (value != null) {
					if (value instanceof IntegerTerm) {
						...
						throw new NoAnswerException(
								"Answer is not an integer: (" + value
										+ ") but List is:" + list);
					}
				} else {
					throw new NoAnswerException(
							"Answer null when it should not be null");
				}
	...
	/**
	 * Ensure that we have an environment and have loaded the prolog code and
	 * have an interpreter to use.
	 */
	private synchronized static void setup() {
		...
		env.ensureLoaded(AtomTerm.get(MentalArithmetic.class.getClassLoader()
				.getResource("dev/mentalarithmetic.pl").getFile()));
	...
	/**
	 * Collect debugging information if something has gone wrong in particular
	 * get any {@link PrologTextLoaderError PrologTextLoaderErrors} which were
	...
	private static void debug() {
		...
		/*
		 * Map<AtomTerm, Term> atom2flag = env.getPrologFlags(); Set<AtomTerm>
		 * atoms = atom2flag.keySet(); for (AtomTerm a : atoms) {
